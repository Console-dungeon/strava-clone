...
\begin{titlepage}
    ...
    \vspace{0.5cm}
    {\large Aplikacja do śledzenia aktywności fizycznej\par}
...
\begin{lstlisting}[frame=single, basicstyle=\ttfamily\footnotesize, numbers=none]
 ...
 | user_id (FK)         |
 | type                 |
 | distance             |       activity_gpx
 | duration             |  +--------------------+
 | avg_speed            |  | id (PK)            |
 | avg_hr / max_hr      |  | activity_id (FK)   |
 | max_speed            |<-+ route_points (JSON) |
 | calories             |  | elevation_gain     |
 | garmin_activity_id   |  +--------------------+
 +----------------------+
\end{lstlisting}
\caption{Uproszczony diagram relacji encji}
\end{figure}
...
\item \textbf{Cache danych GPX} \\
Dane GPS dla aktywności Garmin są cache'owane po pierwszym pobraniu, eliminując powtórne zapytania do API Garmin przy kolejnych wyświetleniach mapy.
