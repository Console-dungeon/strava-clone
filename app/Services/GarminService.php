...
class GarminService
{
    ...
    public function __construct(string $email, string $password, int $userId)
    {
    ...
    public function syncActivities(User $user, int $limit = 100): int
    {
        ActivityLogger::log(ActivityLogger::GARMIN_SYNC_STARTED, $user->id);

        $existingIds = $user->activities()
            ->whereNotNull('garmin_activity_id')
            ->pluck('garmin_activity_id')
            ->flip()
            ->all();
